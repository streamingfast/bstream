...
func TestForkable_ProcessBlock_WithPartialBlocks(t *testing.T) {
	cases := []struct {
		name                               string
		forkDB                             *ForkDB
		ensureAllBlocksTriggerLongestChain bool
		...
		{
			name:               "happy path with partial blocks",
		...
		{
			name:               "undo wrong lastPartialBlock",
			forkDB:             fdbLinked("00000002a"),
			protocolFirstBlock: 2,
			processBlocks: []*pbbstream.Block{
				bTestBlock("00000003a", "00000002a"),          // block{3}
				partialBlock("00000004b", "00000003a", 1),     // partialBlock{4, idx=1}
				lastPartialBlock("00000004c", "00000003a", 3), // partialBlock{4, idx=3, LAST}
				bTestBlock("00000004a", "00000003a"),          // block{4}   (UNDO: 4c was wrong)
				bTestBlock("00000005a", "00000004a"),          // block{5} -> linked to full block 4a
			},
			expectedResult: []*ForkableObject{
				{
		...
		{
			name:               "undo partial scenario",
			forkDB:             fdbLinked("00000002a"),
			protocolFirstBlock: 2,
			processBlocks: []*pbbstream.Block{
				bTestBlock("00000003a", "00000002a"),
				lastPartialBlock("00000004a", "00000003a", 3),
				partialBlock("00000005b", "00000004a", 1),
				bTestBlock("00000004a", "00000003a"), // this one here should not send event
				bTestBlock("00000005a", "00000004a"),
			},
			expectedResult: []*ForkableObject{
				{
				...
				// no 'step new' here because lastPartial was enough
				//{
				//	step:        bstream.StepNew,
				//	Obj:         "00000004a",
				//	block:       tinyBlk("00000004a"),
			...
			expectResultWithoutPartialBlocks: []*ForkableObject{
				...
				// StepPartial(5b) filtered out
				// StepUndoPartial(5b) filtered out
				{
					step:        bstream.StepNew,
					Obj:         "00000005a",
					block:       tinyBlk("00000005a"),
		...
		},
		{
			...
			processBlocks: []*pbbstream.Block{
				...
				partialBlock("00000004c", "00000003a", 1), // ignored: lower than partial block with idx=2
				partialBlock("00000004a", "00000003a", 3),
				bTestBlock("00000004a", "00000003a"),          // new block replacing old non-last-partial -> sent as NEW
				lastPartialBlock("00000004a", "00000003a", 4), // redundant copy of old partial, not sent
				partialBlock("00000005b", "00000004a", 2),
				lastPartialBlock("00000005a", "00000004a", 3),
				bTestBlock("00000005a", "00000004a"), // this one IS the redundant for 5a
			},
			expectedResult: []*ForkableObject{
				{
					step:        bstream.StepNew,
		...
		},
		{
			...
			expectResultWithoutPartialBlocks: []*ForkableObject{
				...
				// StepPartial(5a) filtered out
			},
		},

		{
			name:               "undo incomplete partialblock sequence",
			forkDB:             fdbLinked("00000002a"),
			protocolFirstBlock: 2,
			processBlocks: []*pbbstream.Block{
				bTestBlock("00000003a", "00000002a"),      // block{3}
				partialBlock("00000004b", "00000003a", 1), // partialBlock{4, idx=1}
				bTestBlock("00000004a", "00000003a"),      // block{4}   (UNDO: 4b may have been wrong, we don't know, because it was not 'lastPartial')
				bTestBlock("00000005a", "00000004a"),      // block{5} -> linked to full block 4a
			},
			expectedResult: []*ForkableObject{
				{
			...
			expectResultWithoutPartialBlocks: []*ForkableObject{
				...
				// StepPartial(4b) filtered out
				// StepUndoPartial(4b) filtered out
				{
					step:        bstream.StepNew,
					Obj:         "00000004a",
					block:       tinyBlk("00000004a"),
		...
		{
			name:               "ignore disordered priority",
			forkDB:             fdbLinked("00000002a"),
			protocolFirstBlock: 2,
			processBlocks: []*pbbstream.Block{
				bTestBlock("00000003a", "00000002a"),           // block{3}
				partialBlock("00000004b", "00000003a", 2),      // partialBlock{4, idx=2}
				partialBlock("00000004d", "00000003a", 6),      // partialBlock{4, idx=6}
				partialBlock("00000004c", "00000003a", 5),      // partialBlock{4, idx=5} -> ignored
				lastPartialBlock("00000004a", "00000003a", 10), // partialBlock{4, idx=10, LAST}
			},
			expectedResult: []*ForkableObject{
				{
					step:        bstream.StepNew,
		...
		{
			name:               "do not link over non-last partial blocks",
			forkDB:             fdbLinked("00000002a"),
			protocolFirstBlock: 2,
			processBlocks: []*pbbstream.Block{
				bTestBlock("00000003a", "00000002a"),      // block{3}
				partialBlock("00000004b", "00000003a", 1), // partialBlock{4, idx=1}
				partialBlock("00000005b", "00000004b", 1), // partialBlock{4, idx=1} -> will not link over partial block 4b
				bTestBlock("00000005a", "00000004b"),      // block{5} -> -> will not link over partial block 4b
			},
			expectedResult: []*ForkableObject{
				{
					step:        bstream.StepNew,
			...
			expectResultWithoutPartialBlocks: []*ForkableObject{
				...
				// StepPartial(4b) filtered out — 5b and 5a do not link over non-last partial
			},
		},
		{
			name:               "link over Last partial blocks",
			forkDB:             fdbLinked("00000002a"),
			protocolFirstBlock: 2,
			processBlocks: []*pbbstream.Block{
				bTestBlock("00000003a", "00000002a"),           // block{3}
				lastPartialBlock("00000004b", "00000003a", 10), // partialBlock{4, idx=10, LAST}
				partialBlock("00000005b", "00000004b", 1),      // partialBlock{5, idx=1}
			},
			expectedResult: []*ForkableObject{
				{
					step:        bstream.StepNew,
		...
		{
			name:               "replace early partial will cause UNDO",
			forkDB:             fdbLinked("00000002a"),
			protocolFirstBlock: 2,
			processBlocks: []*pbbstream.Block{
				bTestBlock("00000003a", "00000002a"),      // block{3}
				partialBlock("00000004b", "00000003a", 3), // partialBlock{4, idx=3}
				bTestBlock("00000004a", "00000003a"),      // block{4} -> replacing a non-final partial block, we want to undo whatever was sent
				bTestBlock("00000005a", "00000004a"),      // block{5} -> linked to real block 4a
			},
			expectedResult: []*ForkableObject{
				{
					step:        bstream.StepNew,
		...
		{
			name:               "dont link full block on partial",
			forkDB:             fdbLinked("00000002a"),
			protocolFirstBlock: 2,
			processBlocks: []*pbbstream.Block{
				bTestBlock("00000003a", "00000002a"),
			...
			expectResultWithoutPartialBlocks: []*ForkableObject{
				...
				// StepPartial(4b) filtered out — 5a does not link over non-last partial
			},
		},

		{
			name:               "link full block on LAST partial",
			forkDB:             fdbLinked("00000002a"),
			protocolFirstBlock: 2,
			processBlocks: []*pbbstream.Block{
				bTestBlock("00000003a", "00000002a"),
				lastPartialBlock("00000004b", "00000003a", 1),
				bTestBlock("00000005a", "00000004b"),
			},
			expectedResult: []*ForkableObject{
				{
		...
		},
		{
			...
			processBlocks: []*pbbstream.Block{
				...
				bTestBlock("00000005a", "00000004b"),
				bTestBlock("00000004b", "00000003a"),
				lastPartialBlock("00000006a", "00000005a", 1),
			},
			expectedResult: []*ForkableObject{
				{
					step:        bstream.StepNew,
			...
			// filterSteps is already StepsAllWithoutPartial for this case, so both runs are identical
			expectResultWithoutPartialBlocks: []*ForkableObject{
				{
					step:        bstream.StepNew,
					Obj:         "00000003a",
		...
		{
			name:               "never send partial blocks in STALLED steps",
			forkDB:             fdbLinked("00000002a"),
			protocolFirstBlock: 2,
			processBlocks: []*pbbstream.Block{
				bTestBlock("00000003b", "00000002a"),
			...
			expectResultWithoutPartialBlocks: []*ForkableObject{
				...
				// StepPartial(4c) filtered out
				// StepUndoPartial(4c) filtered out
				{
					step:        bstream.StepNew,
					Obj:         "00000004b",
					block:       tinyBlk("00000004b"),
	...
	for _, c := range cases {
		c := c
		t.Run(c.name, func(t *testing.T) {
			// Capture initial lib ref before either run can mutate the forkDB.
			var initialLibRef bstream.BlockRef
			if c.forkDB.HasLIB() {
				initialLibRef = c.forkDB.libRef
			}

			runs := []testRun{
				{"with_partial_blocks", true, c.expectedResult},
				{"without_partial_blocks", false, c.expectResultWithoutPartialBlocks},
			}

			for _, run := range runs {
				run := run
				t.Run(run.name, func(t *testing.T) {
					handler := newTestForkableSink(c.undoErr, c.newErr)
					bstream.GetProtocolFirstStreamableBlock = c.protocolFirstBlock

					// Create a fresh forkDB for each sub-run so runs are independent.
					freshFDB := NewForkDB()
					if initialLibRef != nil && !bstream.IsEmpty(initialLibRef) {
						freshFDB.InitLIB(initialLibRef)
					}

					var filterStep bstream.StepType
					if run.withPartial {
						filterStep = bstream.StepsAllWithPartial
					} else {
						filterStep = bstream.StepsAllWithoutPartial
					}

					forkable := New(handler, WithFilters(filterStep))
					forkable.forkDB = freshFDB
					if freshFDB.HasLIB() {
						forkable.lastLIBSeen = freshFDB.libRef
					}
					forkable.ensureAllBlocksTriggerLongestChain = c.ensureAllBlocksTriggerLongestChain
					forkable.includeInitialLIB = c.includeInitialLIB
					// Only apply case-level filterSteps override on the "with partial" run,
					// since the "without partial" run always uses StepsAllWithoutPartial.
					if run.withPartial && c.filterSteps != 0 {
						forkable.filterSteps = c.filterSteps
					}

					var err error
					for _, blk := range c.processBlocks {
						err = forkable.ProcessBlock(blk, blk.Id)
						if err != nil {
							break
						}
					}
					...
					resultsString := ""
					for _, forkableObj := range handler.results {
						resultsString += fmt.Sprintf("- [%s] (%s)\n", forkableObj.block.ID(), forkableObj.step.String())
					}

					require.Equal(t, len(run.expectedResult), len(handler.results), resultsString)

					for i, forkableObj := range handler.results {
						assert.Equal(t, run.expectedResult[i].step, forkableObj.step, i)
						assert.Equal(t, run.expectedResult[i].block.ID(), forkableObj.block.ID(), i)
						assert.Equal(t, run.expectedResult[i].block.Num(), forkableObj.block.Num(), i)
						if run.expectedResult[i].parentBlock != nil {
							assert.Equal(t, run.expectedResult[i].parentBlock.String(), forkableObj.parentBlock.String())
						}
					}
					if t.Failed() {
...
func TestForkable_Linkable(t *testing.T) {
	cases := []struct {
		name               string
		forkDB             *ForkDB
		processBlocks      []*pbbstream.Block
		checkBlock         *pbbstream.Block
		expected           bool
		protocolFirstBlock uint64
	}{
		{
			...
			checkBlock: bTestBlock("00000004a", "00000003a"),
			expected:   true,
		},
		{
			name:               "unhappy path",
			...
			processBlocks: []*pbbstream.Block{
				partialBlock("00000003b", "00000002a", 1),
			},
			checkBlock: bTestBlock("00000004a", "00000003b"),
			expected:   false,
		},
	}
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			handler := newTestForkableSink(nil, nil)
			bstream.GetProtocolFirstStreamableBlock = c.protocolFirstBlock

			forkable := New(handler, WithFilters(bstream.StepsAllWithPartial))
			forkable.forkDB = c.forkDB
			if forkable.forkDB.HasLIB() {
				forkable.lastLIBSeen = forkable.forkDB.libRef
			}
			...
			assert.Equal(t, c.expected, forkable.Linkable(c.checkBlock))
...
func TestForkableSentChainSwitchSegments(t *testing.T) {
	...
	undos, partialUndos, redos, _ := p.sentChainSwitchSegments("00000003a", &ForkableBlock{}, "00000003a")
	assert.Nil(t, undos)
	assert.Nil(t, partialUndos)
	assert.Nil(t, redos)
}

...
- `MultiplexedSourceWithRetryIntervals`: new option setting, per source, the minimum time between two connection attempts. Useful for a rescuer or fallback source that is down most of the time and would otherwise be redialed every 5s.
...
- `FileSource` no longer stalls when a merged-blocks file cannot be read (a zero-byte or otherwise corrupted file). The goroutine reading a file calls `Shutdown()` on failure without ever closing that file's blocks channel, and the consuming loop was a plain `range` over it, so depending on which of the two won the race the request could hang forever instead of returning the error. The loop now selects on `Terminating()` as well.
